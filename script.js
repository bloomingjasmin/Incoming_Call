...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function handleScrollBasedAudio() {
    ...
    const volume = Math.max(0, Math.min(baseVolume, baseVolume * (1 - clampedProgress)));
    ...
    if (ringtone) {
      ...
      if (volume > 0 && ringtone.paused) {
        ...
        // The volume will be 0 but audio will continue playing to allow a smoother fade-out experience, rather than stopping abruptly
  ...
  function initChoiceSection() {
    ...
    function resetChoiceState() {
      // Reset to neutral gradient 
      updateGradientBackground(0.5); 
      if (thunderAudio) {
        setAudioVolume(thunderAudio, 0);
        thunderAudio.pause();
      }
      ...
      letGoBtn.addEventListener('click', () => {
        ...
        if (userText !== '') {
          ...
          setTimeout(() => {
            ...
            releaseMsg.textContent = 'You created space for the present.';
